...
if __name__ == '__main__':
    blocks = pd.read_csv('prediction.csv', index_col=0)
    ...
    make_plot(blocks.Difference, colors='Reds', title='Places getting better')
#     # make_plot(blocks.IncomePerCap, colors='inferno', title='Per Capita Income ($)')
#     # blocks.to_csv('block.csv', encoding='utf-8')
#     # print(blocks.Asian)

# census = []
# with open('prediction.csv', "r", encoding='utf-8') as fn:
...
# print('高度危险街区：')
# for item in census:
#     if float(item['Weight']) > 3:
#         print(item)

# print("治安改善区域：")
# for item in census:
#     if float(item['Difference']) > 31:
#         print(item)

# print('情况恶化街区：')
# for item in census:
#     if float(item['Difference']) < -35:
#         print(item)
